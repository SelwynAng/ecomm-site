class AdminController < ApplicationController
  ...
  def index
    ...
    @revenue_by_day_past_week = @orders_by_day.map { |day, orders| [day.strftime("%A"), orders.sum(&:total)] }

    if @revenue_by_day_past_week.count < 7
      days_of_week = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]

      data_hash = @revenue_by_day_past_week.to_h
      ...
      @revenue_by_day_past_week = complete_ordered_array_with_current_last
    end
    ...
    dates_last_month = (start_date..end_date).to_a
    orders_last_month = Order.where(created_at: month_start..Time.now)
    grouped_orders_by_day = orders_last_month.group_by { |order| order.created_at.to_date }

    revenue_by_day_last_month = dates_last_month.map do |date|
      [date, grouped_orders_by_day.fetch(date, []).sum(&:total)]
    end
